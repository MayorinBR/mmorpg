...
Currently implemented in this project: the basic-attack block chance (`PlayerBlockController`, an `IDamageModifier` hook — 25% chance, full negation by default, tunable per prefab) and the Bash, Magnum Break, Sword Mastery and Two-Handed Sword Mastery skills below.

## Stat Bonuses at Job Level 50 (reference only — not implemented)

| STR | AGI | VIT | INT | DEX | LUK |
|-----|-----|-----|-----|-----|-----|
| +7  | +2  | +4  | +0  | +3  | +2  |

Real Ragnarok Online grants these as automatic bonus stats on top of a Swordman's spent points, by Job Level 50. This project has no equivalent mechanic yet — `PlayerJobProgress` tracks Job Level and grants skill points, but nothing currently grants a class-specific stat bonus for reaching a Job Level milestone. See "Missing systems" below.
...
| **Two-Handed Sword Mastery** | Passive, Lv 1–10 | +4–40 ATK with two-handed swords | **Implemented this pass.** New `TwoHandSwordMastery.asset`, identical mechanism to Sword Mastery above, gated on `WeaponSubtype.TwoHandSword` instead. No two-handed sword item asset exists yet to equip and test it against — only the generic `TestWeapon`/`TestWeaponOff` one-handed test items were tagged with a subtype this pass. |
...
5. ~~Weapon subtypes~~ **+ monster Size** — **weapon subtypes implemented this pass** (`ItemDefinition.WeaponSubtype`, alongside the existing Melee/Ranged `WeaponType`: Dagger, One-Hand Sword, Two-Hand Sword, One-Hand Axe, Two-Hand Axe, Mace, Spear, Bow, Book — one per current base class's iconic weapon). This unblocked Sword Mastery vs. Two-Handed Sword Mastery. The broader weapon-vs-monster-size damage table is still blocked, now solely by the missing monster Size classification (no enemy in this project has one yet).
6. **Class-specific Job Level stat bonuses** — a per-class table of bonus stats granted at Job Level milestones (e.g. Swordman's +7 STR/+2 AGI/+4 VIT/+3 DEX/+2 LUK at Job 50), separate from normal spendable stat points. Not started.

Items 2-4 and the monster Size half of item 5 aren't Swordman-specific — each will very likely come up again for the other five classes, so building any one of them now front-loads work rather than only unblocking Swordman. Adding a real (non-test) sword item asset with `WeaponSubtype` set is a smaller, Swordman-specific follow-up worth doing separately, so Sword Mastery and Two-Handed Sword Mastery have something in-game to actually equip and feel.
